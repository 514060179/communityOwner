    ...
    <view class="content_box">
      <view class="address-list" v-for="(address, index) in addressList" :key="index" @tap="useAddress(address)">
        <view class="top flex justify-start">
          ...
          <text class="tag" v-if="address.isDefault === 'T'">{{ $t('默认-IwS') }}</text>
        </view>
        <view class="detail">{{ address.address }}</view>
        <button class="cu-btn set-btn" @tap.stop="jump('/pages/address/addressEdit', address)">{{ $t('编辑-uTL') }}</button>
      </view>
    </view>
    <view class="foot_box flex justify-around">
      <button class="cu-btn add-btn" @tap="jump('/pages/address/addressEdit', { addressId: '-1' })">{{ $t('添加收货地址-9gb') }}</button>
    </view>
  </view>
</template>

<script>
import { getCurOwner, getUserAddress } from '../../api/owner/ownerApi.js'
export default {
  components: {},
  data() {
    return {
