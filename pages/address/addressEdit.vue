  ...
  <view>
    <view class="content_box">
      <view class="cu-form-group">
        <view class="title">{{ $t('收货人-xUg') }}</view>
        <input v-model="username" :placeholder="$t('请输入收货人-L6n')" class="text-right" />
      </view>

      <view class="cu-form-group">
        <view class="title">{{ $t('手机号-9dl') }}</view>
        <input v-model="tel" :placeholder="$t('请输入手机号-Cdr')" type="number" class="text-right" />
      </view>
      <view class="cu-form-group arrow">
        <view class="title">{{ $t('市-f8V') }}/{{ $t('区-GEp') }}</view>
        <pickerAddress @change="change" class="text-right" style="width: 80%">{{ areaName || $t('请选择地区-Hrr') }}</pickerAddress>
        <text class="cuIcon-right"></text>
      </view>
      <view class="cu-form-group margin-top">
        <textarea v-model="address" :placeholder="$t('请输入详细地址-jII')"></textarea>
      </view>
      <view class="default-box flex justify-between align-center">
        <text class="title">{{ $t('设为默认地址-6iI') }}</text>
    ...
    <view v-if="addressId != 0" class="foot_box flex justify-between">
      <button class="cu-btn delete-btn" @tap="deleteAddress">{{ $t('删除收货地址-Wxt') }}</button>
      <button class="cu-btn save-btn" @tap="editAddress">{{ $t('保存收货地址-ews') }}</button>
    </view>
    <view v-else class="foot_box">
      <button class="cu-btn add-btn" @tap="editAddress">{{ $t('保存收货地址-ews') }}</button>
    </view>
    <shopro-picker ref="shoproCityPicker" :pickerValueDefault="cityPickerValueDefault" @onCancel="onCancel" @onConfirm="onConfirm" />
  </view>
</template>
...
export default {
  ...
  onLoad(options) {
    ...
    getCurOwner()
      ...
      .then(data => {
        ...
        ;(that.addressId = _data.addressId), (that.username = _data.username)
  ...
  methods: {
    ...
    editAddress: function () {
      ...
      saveUpdateUserAddress(data).then(
        data => {
          uni.navigateBack({
            delta: 1
          })
