<template>
  <view>
    <view class="cu-list menu">
      <view class="cu-item">
        <view class="content">
          <text class="text-grey">{{ $t('当前小区-IUp') }}</text>
        </view>
        <view class="action">
      ...
      <view class="cu-list menu">
        <view class="cu-item" v-for="(item, idx) in locations" :key="idx">
          <view class="content padding-tb-sm">
            <view> <text class="cuIcon-clothesfill text-blue margin-right-xs"></text> {{ item.locationObjName }}</view>
            <view class="text-gray text-sm"> <text class="cuIcon-infofill margin-right-xs"></text> {{ item.machineCode }}</view>
          </view>
        </view>
      </view>
      <view class="akl_wirte_btn">
        <view class="padding flex flex-direction">
          <button class="cu-btn bg-green lg" @click="gotoApplyApplicationKeyPage()">{{ $t('填写资料-qm7') }}</button>
        </view>
      </view>
    </view>
    <view v-else>
...
export default {
  ...
  methods: {
    ...
    gotoApplyApplicationKeyPage: function () {
      ...
      wx.navigateTo({
        url: '/pages/applicationKey/applicationKeyUser?locations=' + JSON.stringify(this.locations) + '&communityId=' + this.communityId
      })
    }
  }
}
