<template>
  <view>
    ...
    <view class="cu-form-group">
      ...
      <input v-model="name" required :label="$t('姓名-rTx')" clearable :placeholder="$t('请输入名称-PaE')" name="name" />
    </view>
    <view class="cu-form-group">
      <view class="title">{{ $t('年龄-bp9') }}</view>
      <input v-model="age" type="number" :label="$t('年龄-bp9')" :placeholder="$t('请输入年龄-weM')" required clearable name="age" />
    </view>
    <view class="cu-form-group">
      <view class="title">{{ $t('性别-psV') }}</view>
      <picker @change="PickerChange" :value="sex" :range="sexList">
        <view class="picker">
          {{ index > -1 ? sexList[index] : $t('请选择性别-Vh4') }}
        </view>
      </picker>
    </view>
    ...
    <view class="cu-form-group">
      <view class="title">{{ $t('身份-4AM') }}</view>
      <picker :value="typeCdName" @change="PickerIdentChange" :range="typeCdList">
        <view class="picker" @change="onChange" @cancel="onCancel" @confirm="onConfirm">
          {{ indexIdent > -1 ? typeCdList[indexIdent] : $t('请选择身份-X7g') }}
        </view>
      </picker>
    </view>
    <view class="cu-form-group">
    ...
    <view class="cu-form-group">
      <view class="title">{{ $t('有效期-6gU') }}</view>
      <picker :value="expiry" @change="PickerExpiryChange" :range="expiryList">
        <view class="picker">
          {{ indexExpiry > -1 ? expiryList[indexExpiry] : $t('请选择有效期-XkN') }}
        </view>
      </picker>
    </view>
    ...
    <view class="cu-form-group">
      ...
      <input v-model="tel" type="number" required :label="$t('手机号-9dl')" clearable :placeholder="$t('请输入手机号-Cdr')" name="tel" />
    </view>
    <view class="cu-form-group">
      <view class="title">{{ $t('验证码-dkC') }}</view>
      <input v-model="msgCode" :placeholder="$t('请输入短信验证码-YxH')" name="input" />
      <button class="cu-btn bg-green shadow" type="number" :disabled="btnDisabled" @click="sendMsgCode()">{{ $t('验证码-dkC') }}</button>
    </view>

    <view class="block__title">{{ $t('证件照片-cLM') }}</view>
    ...
    <view class="padding flex flex-direction">
      <button class="cu-btn bg-green lg" @click="saveApplicationKey()">{{ $t('提交申请-sBk') }}</button>
    </view>
    <view class="button_up_blank"></view>
  </view>
</template>
...
import { formatTime, addMonth, addYear } from '../../lib/proprietor/utils/DateUtil.js'
...
export default {
  ...
  onLoad: function (options) {
    ...
    getProperty().then(function (_property) {
  ...
  methods: {
    ...
    saveApplicationKey: function () {
      ...
      if (_objData.sex != 0 && _objData.sex != 1) {
        msg = this.$t('请选择性别-Vh4')
      }

      if (_objData.typeCd == '') {
        msg = this.$t('请选择身份-X7g')
      }

      if (_objData.idCard == '') {
        msg = this.$t('请填写身份证-Jgh')
      }

      if (_objData.startTime == null || _objData.endTime == null) {
        msg = this.$t('请选择有效期-XkN')
      }

      if (_objData.tel == '') {
        msg = this.$t('请选择填写手机号-EsI')
      }

      if (_objData.photos == null || _objData.photos.length < 2) {
        msg = this.$t('请选择证件照片-sma')
      }
