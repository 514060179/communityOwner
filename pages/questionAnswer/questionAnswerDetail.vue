<template>
  <view>
    ...
    <view class="flex justify-between padding-lr-sm margin-top-xs">
      <view>{{ $t('问卷人姓名-oKX') }}（{{ $t('业主-FFN') }}）</view>
      <view>{{ qaInfo.ownerName }}</view>
    </view>
    <view class="flex justify-between padding-lr-sm margin-top-xs">
      <view>{{ $t('联系电话-Gao') }}</view>
    ...
    <view class="" v-for="(item, index) in titles" :key="index">
      ...
      <radio-group class="block" @change="radioChange($event, item)" v-if="item.titleType == '1001'">
        <view class="cu-form-group" v-for="(valueItem, valueIndex) in item.titleValues" :key="valueIndex">
          ...
          <radio :class="item.radio == valueItem.valueId ? 'checked' : ''" :checked="item.radio == valueItem.valueId ? true : false" :value="valueItem.valueId"> </radio>
        </view>
      </radio-group>
      ...
      <view v-else>
        <view class="cu-form-group">
          <textarea maxlength="512" v-model="item.radio" :placeholder="$t('请回答-b1F')"></textarea>
        </view>
      </view>
    </view>
    <view class="button_up_blank"></view>
    <view class="flex flex-direction">
      <button class="cu-btn bg-green margin-tb-sm lg" @click="submitQuestionAnswer()">{{ $t('提交-4TX') }}</button>
    </view>
  </view>
</template>
...
import { queryQuestionAnswer, queryQuestionAnswerTitle, saveUserQuestionAnswerValue } from '../../api/question/questionApi.js'
...
export default {
  ...
  methods: {
    ...
    submitQuestionAnswer: function (e) {
      ...
      saveUserQuestionAnswerValue(obj).then(
        _data => {
          uni.showToast({
            icon: 'none',
            title: this.$t('提交成功-Csi')
          })
          uni.navigateBack({
            delta: 1
          })
