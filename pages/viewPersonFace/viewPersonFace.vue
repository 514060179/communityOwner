<template>
  <view class="vpf_context">
    <view class="vpf_row">
      <view class="vpf_row_black"> </view>
      <view class="vpf_row_title">
        <text>{{ $t('人脸信息-gtb') }}</text>
      </view>
      ...
      <view class="padding flex flex-direction">
        <button class="cu-btn bg-green lg" @click="collectFace()">{{ $t('采集人脸-fGo') }}</button>
      </view>
...
export default {
  ...
  methods: {
    collectFace: function () {
      ...
      uni.chooseImage({
        ...
        success: function (res) {
          const _photosPromise = factory.base64.urlTobase64(res.tempFilePaths[0])
          _photosPromise.then(
            photo => {
    ...
    loadOwnerFace: function () {
      ...
      context.getOwner(function (_owner) {
        const _face = constant.url.getOwnerPhotoPath + '?objId=' + _owner.memberId + '&communityId=' + _owner.communityId + '&fileTypeCd=10000&time=' + new Date()
