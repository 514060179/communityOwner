  ...
  <view>
    <view>
      ...
      <view v-if="noData == false" style="margin-top: 110upx">
        <view v-for="(item, index) in parkings" :key="index" class="bg-white margin-bottom margin-right-xs radius margin-left-xs padding padding-bottom-xs">
          ...
          <view class="flex margin-top-xs justify-between">
            <view class="text-gray">{{ $t('车辆品牌-OAN') }}</view>
            <view class="text-gray">{{ item.carBrand }}</view>
          </view>
          <view class="flex margin-top-xs justify-between">
            <view class="text-gray">{{ $t('起租时间-D8T') }}</view>
            <view class="text-gray">{{ item.createTime }}</view>
          </view>
          <view class="flex margin-top-xs justify-between">
            <view class="text-gray">{{ $t('结租时间-PSR') }}</view>
          ...
          <view class="solid-top flex justify-end margin-top padding-top-sm padding-bottom-sm" v-if="item.state == '2002'">
            <button class="cu-btn sm bg-green margin-left" @click="toPayFee(item)">{{ $t('去缴费-LGJ') }}</button>
          </view>
        </view>
      </view>
      <view v-else>
