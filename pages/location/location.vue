  ...
  <view>
    <view class="header">
      <view class="search-input">
        <input :placeholder="$t('请输入小区名称-OgY')" @input="input" />
      </view>
      <view class="search-btn">{{ $t('搜索-WEd') }}</view>
    </view>
    <view class="result-container" :hidden="hidden">
      <scroll-view scroll-y="true" class="search-result-list" :hidden="hidden">
        <block v-for="(item, index) in locationList" :key="index">
          <view class="search-result" @tap="onTap" :data-key="item.address">
            {{ item.name }}
    ...
    <view class="getLocation" @tap="getLocation">{{ $t('点击定位当前位置-Sw5') }}</view>
    <view class="addLocation">{{ $t('新增收货地址-3YL') }}<view class="addLocation-icon">+</view> </view>
    <view class="myLocation">{{ $t('我的收货地址-ofT') }}</view>
