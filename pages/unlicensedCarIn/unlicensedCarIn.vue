<template>
  <view>
    <view class="plate-context">
      ...
      <view v-if="showFlag == 'phone'">
        ...
        <view class="plate-input-body">
          <view class="plate-input-content">
            <input type="number" maxlength="11" class="input-ui" v-model="carNum" :placeholder="$t('手机号作为车牌号入场-8rW')" />
          </view>
        </view>
      </view>
      ...
      <view class="cu-bar btn-group" style="margin-top: 30px">
        <button @click="_saveCarNum" :disabled="carNum == ''" class="cu-btn bg-green shadow-blur round lg">{{ $t('立即入场-B7S') }}</button>
      </view>
    </view>
    ...
    <view class="cu-modal" :class="showToastFlag == true ? 'show' : ''">
      <view class="cu-dialog">
        ...
        <view class="padding-xl"> {{ $t('车牌号提交成功-Rxa') }}! </view>
        <view class="cu-bar bg-white justify-end">
          <view class="action margin-0 flex-sub solid-left" @tap="_closeToastModal()">{{ $t('关闭-KJs') }}</view>
        </view>
      </view>
    </view>
...
// import { refreshUserOpenId } from '../../api/user/userApi.js'
