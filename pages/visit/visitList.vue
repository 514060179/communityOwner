<template>
  <view>
    <view class="block__title">
      <text>{{ $t('访客预约历史-qGU') }}</text>
      <button class="cu-btn bg-primary" @click="_addVisit()">{{ $t('新增预约-72Q') }}</button>
    </view>
    <view v-if="noData == false">
      <view v-for="(item, index) in visits" :key="index" class="bg-white margin-bottom margin-right-xs radius margin-left-xs padding" @click="_showDetail(item)">
        <view class="flex padding-bottom-xs solid-bottom justify-between">
          <view>{{ $t('预约状态-9Rq') }}</view>
          <view class="text-gray" v-if="item.state == 0 || item.state == 2 || item.state == 3">{{ $t('未生效-dpb') }}</view>
          <view class="text-gray" v-else>{{ item.recordState == 0 ? $t('已生效-BRt') : $t('已失效-jdM') }}</view>
        </view>
        <view class="flex margin-top justify-between">
          <view class="text-gray">{{ $t('访客姓名-VVL') }}</view>
        ...
        <view class="flex margin-top-xs justify-between" v-if="item.carNum">
          <view class="text-gray">{{ $t('车牌号-c9W') }}</view>
          <view class="text-gray">{{ item.carNum }}</view>
        </view>
        <view class="flex margin-top-xs justify-between" v-if="item.carNum">
          <view class="text-gray">{{ $t('车辆审核状态-Ioo') }}</view>
        ...
        <view class="flex margin-top-xs justify-between">
          <view class="text-gray">{{ $t('预计来访时间-mvW') }}</view>
...
export default {
  ...
  methods: {
    ...
    _loadVisitList: function (isRefresh) {
      ...
      listOwnerVisit(_objData).then(function (_visits) {
        _that.visits = isRefresh ? (_visits.visits || []) : _that.visits.concat(_visits.visits || [])
        _that.page++
