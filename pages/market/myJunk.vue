  ...
  <view>
    <scroll-view scroll-x class="bg-white nav">
      <view class="flex text-center">
        <view class="cu-item flex-sub" :class="active == 0 ? 'text-green cur' : ''" @tap="_tabSelect(0)"> {{ $t('我的旧货-QtM') }} </view>
        <view class="cu-item flex-sub" :class="active == 1 ? 'text-green cur' : ''" @tap="_tabSelect(1)"> {{ $t('我的需求-z9A') }} </view>
      </view>
    </scroll-view>
    <view v-if="active == 0">
    ...
    <view class="cu-modal" :class="deleteJunkModal == true ? 'show' : ''">
      <view class="cu-dialog">
        ...
        <view class="padding-xl"> {{ $t('您确认删除吗？-vH3') }} </view>
        <view class="cu-bar bg-white justify-end">
          <view class="action margin-0 flex-sub solid-left" @tap="_cancleDeleteJunk()">{{ $t('取消-KvD') }}</view>
          <view class="action margin-0 flex-sub text-green solid-left" @tap="_doDeleteJunk()">{{ $t('确定-vdT') }}</view>
        </view>
      </view>
    </view>
    ...
    <view class="cu-modal" :class="finishJunkModal == true ? 'show' : ''">
      <view class="cu-dialog">
        ...
        <view class="padding-xl"> {{ $t('您确认标记为已完成吗？-Df9') }} </view>
        <view class="cu-bar bg-white justify-end">
          <view class="action margin-0 flex-sub solid-left" @tap="_cancleFinishJunk()">{{ $t('取消-KvD') }}</view>
          <view class="action margin-0 flex-sub text-green solid-left" @tap="_doFinishJunk()">{{ $t('确定-vdT') }}</view>
        </view>
      </view>
    </view>
  </view>
