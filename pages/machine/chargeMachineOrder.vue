  ...
  <view class="">
    <view class="margin border-radius">
      <view class="cu-list menu">
        ...
        <view class="cu-item">
          <view class="content">
            <text class="text-grey">{{ $t('充电时间-HpP') }}</text>
          </view>
          <view class="action">
            {{ chargeHours || 0 }}
          </view>
        ...
        <view class="cu-item">
          <view class="content">
            <text class="text-grey">{{ $t('充电桩-D1h') }}</text>
          </view>
          <view class="action">
            {{ machineName || 0 }}
          </view>
        ...
        <view class="cu-item">
          <view class="content">
            <text class="text-grey">{{ $t('充电插槽-RUs') }}</text>
          </view>
          <view class="action">
            {{ portName || 0 }}
          </view>
        ...
        <view class="cu-item">
          <view class="content">
            <text class="text-grey">{{ $t('计费模式-WlH') }}</text>
          </view>
          <view class="action"> {{ durationPrice || 0 }} MOP/{{ $t('小时-DBA') }} </view>
        </view>
        <view class="cu-item">
          <view class="content">
    ...
    <view class="cu-bar btn-group" style="margin-top: 30px" v-if="state == '1001'">
      <button @click="_stopCharge" class="cu-btn bg-orange shadow-blur round lg">{{ $t('结束充电-tS7') }}</button>
    </view>
  </view>
</template>

<script>
import { stopCharge, getChargeMachineOrder } from '../../api/machine/machineApi.js'
    ...
    _loadChargeMachineOrder: function () {
      ...
      getChargeMachineOrder({
        ...
        if (parseInt(_data[0].chargeHours) == 999) {
          _that.chargeHours = this.$t('充满自停-RNt')
        } else {
