  ...
  <view>
    <view class="face-top" v-if="hasFace == 'N'">
      ...
      <view class="face-desc">
        <text>{{ $t('您尚未进行人脸识别认证，请联系物业公司-ifW') }}</text>
      </view>
    </view>
    <view class="face-top" v-else>
      ...
      <view class="face-desc">
        <text>{{ $t('您已通过人脸识别认证，请放心使用-aaS') }}</text>
      </view>
    </view>

    <view class="face-tel">
      <view v-if="property.communityQrCode"><image class="call-qrcode" :src="property.communityQrCode"></image></view>
      <view class="bg-white face-tel-text">{{ $t('您确认拨打-yat') }},{{ property.communityName }}{{ $t('物业客服电话-NKY') }}<br />{{ property.sCommunityTel }}</view>
    </view>
  </view>
</template>
...
import { hasOwner, loadLoginOwner } from '../../api/owner/ownerApi.js'
  ...
  methods: {
    _loadCurrentOwner: function () {
      ...
      loadLoginOwner({
        ...
        const _attrs = _owner.ownerAttrDtos
        _attrs.forEach(item => {
