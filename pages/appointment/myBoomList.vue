<template>
  <view class="tab-container bg-white">
    <view class="cu-list menu">
      ...
      <view class="header_fixed">
        <scroll-view class="bg-white nav">
          <view class="flex text-center">
            <view class="cu-item flex-sub" :class="item.code == code ? 'text-green cur' : ''" v-for="(item, index) in parkingType" :key="index" @tap="switchParkingSpace(item)" :data-id="index">
              {{ item.name }}
            </view>
          </view>
        </scroll-view>
      ...
      <view v-for="(item, index) in bookingList" :key="index" class="bg-white margin-top margin-right-xs radius margin-left-xs padding">
        <view class="flex padding-bottom-xs justify-between">
          <view style="font-size: 14px">
            {{ $t('单号-JiJ') }}<span style="margin-left: 10px" class="text-gray">{{ item.timeId }}</span>
          </view>
        </view>
        <view class="flex padding-bottom-xs solid-bottom justify-between">
          <view style="font-size: 14px">
            {{ $t('场馆-QP4') }}<span style="margin-left: 10px" class="text-gray">{{ item.venueName }}</span>
          </view>
        </view>
        <view class="flex margin-top justify-between">
          <view class="text-gray">{{ $t('场地-VP1') }}</view>
        ...
        <view class="flex margin-top-xs justify-between">
          <view class="text-gray">{{ $t('预约人-J7K') }}</view>
          <view class="text-gray">{{ item.personName }}</view>
        </view>
        <view class="flex margin-top-xs justify-between">
          <view class="text-gray">{{ $t('预约电话-fmV') }}</view>
        ...
        <view class="flex margin-top-xs justify-between">
          <view class="text-gray">{{ $t('核销时间-J9K') }}</view>
        ...
        <view class="solid-top flex justify-end margin-top padding-top-sm">
          ...
          <button class="cu-btn bg-gradual-blue margin-left" @click="toApplyDetail(item)">{{ $t('详情-6NZ') }}</button>
        </view>
...
export default {
  ...
  methods: {
    ...
    StateStr(s) {
      if (s == 'W') {
        return this.$t('待核销-2uJ')
      }
      if (s == 'C') {
        return this.$t('已核销-quh')
      }
      if (s == 'Y') {
        return this.$t('已退款-cIm')
      }
      if (s == 'T') {
        return this.$t('申请退款-JFp')
      }
    },
    // 时间转换
    HoursStr(hours) {
