    ...
    <view class="cu-list menu" v-if="renovationList.length > 0">
      <view class="cu-item arrow" v-for="(item, key) in renovationList" :key="key" :data-item="item" @click="toRenovationDetail(item)">
        <view class="content padding-tb-sm">
          <view> <text class="cuIcon-homefill text-green margin-right-xs"></text> {{ item.stateName }}</view>
          <view class="text-gray text-sm"> <text class="cuIcon-right margin-right-xs"></text> {{ item.createTime }}</view>
        </view>
    ...
    <view class="cu-list menu" v-else>
      <view class="cu-item">
        <view class="content">
          ...
          <text class="text-grey">{{ $t('暂无装修信息-4jU') }}</text>
        </view>
        <view class="action"> </view>
      </view>
    </view>
...
import { queryRoomRenovation } from '../../api/roomRenovation/roomRenovationApi.js'
