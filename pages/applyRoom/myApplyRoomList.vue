    ...
    <view class="cu-list menu" v-if="applyList.length > 0">
      <view class="cu-item arrow" v-for="(item, key) in applyList" :key="key" :data-item="item" @click="toApplyRoomDetail(item)">
        <view class="content padding-tb-sm">
          <view> <text class="cuIcon-homefill text-green margin-right-xs"></text> {{ item.applyTypeName }}</view>
          <view class="text-gray text-sm"> <text class="cuIcon-right margin-right-xs"></text> {{ item.stateName }}</view>
        </view>
    ...
    <view class="cu-list menu" v-else>
      <view class="cu-item" v-if="applyList.length === 0">
        <view class="content">
          ...
          <text class="text-grey">{{ $t('暂无申请信息-wZy') }}</text>
        </view>
        <view class="action"> </view>
      </view>
    </view>
