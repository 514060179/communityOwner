<template>
  <view>
    <view class="page">
      <view class="tab">
        <view class="tabItem" v-for="(item, index) in tab" :key="index" :class="{ active: active == index }" @click="changeTab(index)">
        ...
        <view>
          <view class="list" v-if="active == 0">
            <view class="listItem" v-for="(item, index) in couponList" :key="index" @click="_toQrCode(item)">
              ...
              <view class="box">
                <view class="descripe">
                  <view class="shop-name">{{ item.couponName }}({{ item.stock }}{{ $t('张-Cp7') }})</view>
                  <view class="text">{{ item.value }}</view>
                  <view class="expire">{{ item.createTime }}-{{ item.endTime }} {{ $t('前-jpu') }}</view>
                </view>
                <view class="usestate">
                  {{ item.toTypeName }}
                </view>
