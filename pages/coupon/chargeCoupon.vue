<template>
  <view>
    <view class="block__title">{{ $t('我的充电劵-GU1') }}</view>
    <!-- 费用项 -->
      ...
      <view class="tl-section">
        <checkbox-group class="block" @change="checkboxChange">
          <view class="cu-form-group" v-for="(coupon, index) in couponList" :key="index">
            <view class="title">{{ coupon.couponName }} {{ $t('面值-jQN') }}：{{ coupon.value }}{{ $t('小时-DBA') }} {{ $t('有效期-6gU') }}：{{ coupon.endTime }} </view>
