  ...
  <view>
    <view class="plate-context">
      ...
      <view class="cu-bar btn-group" style="margin-top: 30px">
        <button @click="_receiveParkingCoupon" :disabled="carNum.length < 7 || getSuccess" class="cu-btn bg-green shadow-blur round lg">{{ $t('立即领取-LSe') }}</button>
      </view>

      <view class="temp-remark">
        <view>{{ $t('停车劵说明-8DM') }}</view>
        <view>1、{{ $t('领取的停车劵请在24小时内使用-J9o') }}：</view>
        <view>2、{{ $t('领取停车劵请在扫码缴费时使用-aoO') }}：</view>
...
import { receiveParkingCoupon } from '../../api/fee/feeApi.js'
  ...
  methods: {
    ...
    _receiveParkingCoupon: function () {
      ...
      receiveParkingCoupon({
        ...
        uni.showToast({
          icon: 'none',
          title: this.$t('停车劵领取成功，请关闭页面-pod'),
          duration: 10000
        })
