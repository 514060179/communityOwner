<template>
  <view>
    <scroll-view scroll-y>
      <view class="block__title">{{ $t('停车费-RR3') }}</view>
      <view class="cu-list menu fee-last">
        <view class="cu-item">
          <view class="content">
            <text class="text-grey">{{ $t('车辆-REb') }}</text>
          </view>
          <view class="action">
        ...
        <view class="cu-item">
          <view class="content">
            <text class="text-grey">{{ $t('查询时间-okP') }}</text>
          </view>
          <view class="action">
        ...
        <view class="cu-item">
          <view class="content">
            <text class="text-grey">{{ $t('进场时间-bDY') }}</text>
          </view>
          <view class="action">
        ...
        <view class="cu-item">
          <view class="content">
            <text class="text-grey">{{ $t('停车时间-CSP') }}</text>
          </view>
          <view class="action">
            <text class="text-grey text-sm">{{ stopTimeTotal }}{{ $t('分钟-EaL') }}</text>
          </view>
        </view>
        <view class="cu-item">
          <view class="content">
        ...
        <view class="cu-list menu margin-top" @click="_selectCoupons">
          <view class="cu-item arrow">
            <view class="content padding-tb-sm">
              <view>
                <view class="text-cut" style="width: 220px">{{ $t('使用停车劵抵扣-KyQ') }}</view>
              </view>
            </view>
        ...
        <view class="cu-bar btn-group" style="margin-top: 20upx">
          <button @click="onPayFee" v-if="amount > 0" class="cu-btn bg-green shadow-blur round lg">{{ $t('确认缴费-Nsi') }}</button>
          <button @click="onPayFee" v-else class="cu-btn bg-green shadow-blur round lg">{{ $t('确认开门-Mbs') }}</button>
        </view>
        <view class="cu-bar btn-group" style="margin-top: 20upx">
          <button @click="onReQuery()" class="cu-btn bg-grey shadow-blur round lg">{{ $t('重新查询-OEK') }}</button>
        </view>
      </view>
      <view class="temp-remark">
        <view>{{ $t('临停缴费提示-rva') }}</view>
        <view>1、{{ $t('支付后请在15分钟内离场，否则会产生新的停车费-UiD') }}：</view>
        <view>2、{{ $t('缴费前请仔细核对停车场、停车时间、支付金额-8wQ') }}。</view>
      </view>
    </scroll-view>
  </view>
</template>
...
import { getTempCarFeeOrder, toPayTempCarFee, toAliPayTempCarFee } from '../../api/fee/feeApi.js'
...
export default {
  ...
  methods: {
    ...
    _dealCarCoupons: function () {
      const carCoupons = uni.getStorageSync(constant.mapping.COUPON_USER_TEMP_CAR_KEY)
      if (!carCoupons) {
        return
      }
    ...
    onAliPayPayFee: function () {
      ...
      toAliPayTempCarFee(_objData).then(_data => {
        if (_data.code == '0') {
          // #ifdef H5
    ...
    onWxPayFee: function () {
      ...
      toPayTempCarFee(_objData).then(_data => {
        wx.hideLoading()
        if (_data.code == '0') {
          // #ifdef H5
