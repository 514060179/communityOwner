  ...
  <view>
    <scroll-view scroll-y>
      ...
      <view class="cu-list menu">
        ...
        <view class="cu-item">
          <view class="content">
            <text class="text-grey">{{ $t('房屋-5Ci') }}</text>
          </view>
          <view class="action">
            <text class="text-grey text-sm">{{ `${floorNum}${$t('号楼-G4C')}${unitNum}${$t('单元-7xd')}${roomNum}${$t('室-AtK')}` }}</text>
          </view>
        </view>
        <view class="cu-item">
          <view class="content">
            <text class="text-grey">{{ $t('建筑面积-7e6') }}</text>
          </view>
          <view class="action">
      ...
      <view class="cu-list menu fee-last">
        ...
        <view class="cu-item" v-if="feeFlag != '2006012'">
          <view class="content">
            <text class="text-grey">{{ $t('缴费后时间-sMI') }}</text>
          </view>
          <view class="action">
        ...
        <view class="cu-item" v-if="preReadingTime">
          <view class="content">
            <text class="text-grey">{{ $t('上期读表时间-a7W') }}</text>
          </view>
          <view class="action">
        ...
        <view class="cu-item" v-if="preDegrees">
          <view class="content">
            <text class="text-grey">{{ $t('上期度数-gBe') }}</text>
          </view>
          <view class="action">
        ...
        <view class="cu-item" v-if="curReadingTime">
          <view class="content">
            <text class="text-grey">{{ $t('本期读表时间-4Gl') }}</text>
          </view>
          <view class="action">
        ...
        <view class="cu-item" v-if="curDegrees">
          <view class="content">
            <text class="text-grey">{{ $t('本期度数-iMK') }}</text>
          </view>
          <view class="action">
        ...
        <view class="cu-item" v-if="curDegrees">
          <view class="content">
            <text class="text-grey">{{ $t('使用量-yit') }}</text>
          </view>
          <view class="action">
            <text class="text-grey text-sm">{{ curDegrees - preDegrees }}</text>
          </view>
        </view>
        <vcUserAccount ref="vcUserAccountRef" @getUserAmount="getUserAmount" v-if="false"></vcUserAccount>
        <!-- <view class="cu-list menu margin-top" @click="coupons()">
          <view class="cu-item arrow">
            <view class="content padding-tb-sm">
              <view>
                <view class="text-cut" style="width: 220px">{{ $t('使用优惠卷抵扣-oVF') }}</view>
              </view>
            </view>
...
export default {
  ...
  methods: {
    _dealUserCoupons: function () {
      const couponUser = uni.getStorageSync(constant.mapping.COUPON_USER_KEY)
      if (!couponUser) {
        return
      }
      ...
      if (this.couponAmount) {
        this.receivableAmount = (parseFloat(this.receivableAmount) - parseFloat(this.couponAmount)).toFixed(2)
    ...
    _computeUserAmount: function () {
      ...
      // 更新应缴金额
      const receivableAmount = parseFloat(this.receivableAmount) - parseFloat(this.accountAmount)
    ...
    _payWxApp: function (_data) {
      ...
      if (this.payOnline == 'N') {
        uni.showToast({
          icon: 'none',
          title: this.$t('暂不支持线上缴费，请到前台缴费-g5Z')
        })
        return
      }
      payFeeApp(this, {
    ...
    onPayFee: function () {
      ...
      if (_receivedAmount < 0) {
        uni.showToast({
          icon: 'none',
          title: this.$t('金额为负数，可能用于减免，请到柜台缴费-egn')
        })
        return
      }
