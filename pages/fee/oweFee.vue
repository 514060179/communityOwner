<template>
  <view>
    <view class="cu-form-group arrow margin-top" v-if="payerObjs && payerObjs.length > 1">
      <view class="title" style="width: 200upx">{{ $t('按房屋缴费-cxj') }}</view>
    ...
    <scroll-view scroll-y style="padding-bottom: 200rpx">
      <view class="block__title" v-if="fees.length > 0">{{ $t('应缴费用-3m7') }}</view>
...
import { formatDate, dateSubOneDay, getDate } from '../../lib/proprietor/utils/DateUtil.js'

// import { getCurCommunity } from '../../api/community/communityApi.js'

import { getUserId } from '../../api/user/userApi.js'

import { getRoomOweFees } from '../../api/fee/feeApi.js'
...
export default {
  ...
  methods: {
    ...
    _payerObjectChange: function (e) {
      ...
      const _curPayerObj = this.payerObjs[e.detail.value]
    ...
    computeReceivableAmount() {
      ...
      _receivableAmount = _receivableAmount < 0 ? 0.0 : _receivableAmount
