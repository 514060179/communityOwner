  ...
  <view>
    <scroll-view scroll-x class="bg-white nav">
      <view class="flex text-center">
        <view class="cu-item flex-sub" :class="active == 0 ? 'text-green cur' : ''" @tap="tabSelect(0)"> {{ $t('综合缴费-O5e') }} </view>
        <view class="cu-item flex-sub" :class="active == 1 ? 'text-green cur' : ''" @tap="tabSelect(1)"> {{ $t('缴费记录-8IN') }} </view>
      </view>
    </scroll-view>
...
<script>
import { getPayFeeQrcode } from '@/api/fee/qrCodePayFee.js'
