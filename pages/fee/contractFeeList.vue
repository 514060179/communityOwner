<template>
  <view>
      ...
      <scroll-view v-if="contracts.length < 5 && contracts.length > 1" class="bg-white nav">
        <view class="flex text-center">
          <view
            ...
            :class="item.contractId == curContract.contractId ? 'text-green cur' : ''"
            v-for="(item, index) in contracts"
            :key="index"
            @tap="switchContract(item)"
            :data-id="index"
          >
            {{ item.contractName }}
          </view>
    ...
    <view v-if="noData == false || moreContracts.length > 0">
      <view class="cu-list menu" v-for="(fee, idx) in moreContracts" :key="idx" :data-item="fee" @click="payFee(fee)">
...
import { getContracts } from '../../api/contract/contractApi.js'
...
export default {
  ...
  methods: {
    ...
    _loadContractFee: function () {
      const _that = this
      const _contract = this.curContract
      const _objData = {
        page: 1,
        row: 30,
      ...
      _that.moreContracts = []
      getRoomFees(_objData, _contract).then(
        moreContracts => {
