<template>
  <view>
    <view class="plate-context">
      ...
      <view class="tips_block"> </view>
      <view class="cu-bar btn-group" style="margin-top: 30px">
        <button @click="_changeFlag('car')" style="margin-left: 0px" class="cu-btn shadow-blur lg" :class="{ 'bg-primary': showFlag == 'car', 'line-blue': showFlag != 'car' }">{{ $t('车牌号-c9W') }}</button>
        <button @click="_changeFlag('phone')" style="margin-right: 0px" class="cu-btn shadow-blur lg" :class="{ 'bg-primary': showFlag == 'phone', 'line-blue': showFlag != 'phone' }">
          {{ $t('手机号-9dl') }}
        </button>
      </view>
      <view v-if="showFlag == 'phone'">
        <view class="tips">
          <text>{{ $t('请输入手机号-Cdr') }}</text>
        </view>
        <view class="plate-input-body">
          <view class="plate-input-content">
            <input type="number" maxlength="11" class="input-ui" v-model="carNum" :placeholder="$t('以手机号入场无牌车，请填手机号出场-qhT')" />
          </view>
        </view>
      </view>
      <view v-if="showFlag == 'car'">
        <view class="tips">
          <text>{{ $t('点击方框输入车牌号-Tyb') }}</text>
        </view>
        <view class="plate-input-body">
          <view class="plate-input-content" @click="showCarNumberKeyboard">
            <view bindtap="showCarNumberKeyboard" data-id="0" :class="inputOnFocusIndex == '0' ? 'plate-nums-foc' : 'plate-nums-first'">
            ...
            <view bindtap="inputClick" data-id="7" class="green-border" :class="inputOnFocusIndex == '7' ? 'plate-nums-foc' : 'plate-nums-first'">
        ...
        <view class="temp-history">
          <view class="padding">
            <text>{{ $t('往期记录-64p') }}</text>
          </view>
          <view class="flex justify-start">
            <view class="padding-left" v-for="(item, index) in carNums" :key="index" @tap="_loadTempFee(item.carNum)">{{ item.carNum }} </view>
          </view>
        </view>
      ...
      <view class="cu-bar btn-group" style="margin-top: 30px">
        <button @click="_queryCarNum" :disabled="carNum.length < 7" class="cu-btn bg-green shadow-blur round lg">{{ $t('立即查询-ygB') }}</button>
      </view>

      <view class="temp-remark">
        <view>{{ $t('临停支付说明-Xfr') }}</view>
        <view>1、{{ $t('若因特殊原因无法查询车辆信息，请联系停车场人工处理-DbT') }}：</view>
        <view>2、{{ $t('查询缴费前请仔细核对停车场及车牌号-d7L') }}：</view>
        <view>3、{{ $t('若长时间无法加载页面，请检查网络是否畅通或扫码重试-p5N') }}。</view>
      </view>
    </view>

    <select-car-num ref="popupCarNumber" type="bottom" @confirm="confirmGuaCarNumber" @getCarNum="getCarNum" :gua="false"></select-car-num>
  </view>
</template>
...
import { refreshUserOpenId, getOpenIdFromAliPay } from '../../api/user/userApi.js'
import { isWxOrAli } from '../../lib/proprietor/utils/EnvUtil.js'

import { queryWaitPayFeeTempCar } from '@/api/car/carApi.js'
export default {
  data() {
    return {
      flag: true,
  ...
  methods: {
    ...
    getCarNum: function (_list) {
      ...
      this.carNum = this.inputPlates.index0 + this.inputPlates.index1 + this.inputPlates.index2 + this.inputPlates.index3
      this.carNum += this.inputPlates.index4 + this.inputPlates.index5 + this.inputPlates.index6 + this.inputPlates.index7
    },
    ...
    _queryCarNum: function () {
      ...
      getTempCarFeeOrder({
        ...
        if (_data.code != 0) {
          uni.showToast({
            icon: 'none',
            title: this.$t('未查到停车费-nCD')
          })
          return
        }
        uni.navigateTo({
          url:
            '/pages/fee/tempCarFee?paId=' +
            _that.paId +
            '&carNum=' +
            _that.carNum +
            '&appId=' +
