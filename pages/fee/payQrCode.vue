  ...
  <view>
    <view class="plate-context">
      ...
      <view class="cu-bar btn-group margin-left margin-right" style="margin-top: 30px">
        <button
          @click="_changeFlag('room')"
          style="margin-left: 0px"
          v-if="config.queryWay == '2002' || config.queryWay == '3003'"
          class="cu-btn shadow-blur lg"
          :class="{ 'bg-primary': queryWay == 'room', 'line-blue': queryWay != 'room' }"
        >
          {{ $t('房屋-5Ci') }}
        </button>
        <button
          @click="_changeFlag('phone')"
          style="margin-right: 0px"
          v-if="config.queryWay == '1001' || config.queryWay == '3003'"
          class="cu-btn shadow-blur lg"
          :class="{ 'bg-primary': queryWay == 'phone', 'line-blue': queryWay != 'phone' }"
        >
          {{ $t('手机号-9dl') }}
        </button>
      </view>
      <view>
        <view class="tips">
          <text v-if="queryWay == 'room'">{{ $t('请输入房号-RzA') }}</text>
          <text v-else>{{ $t('请输入手机号-Cdr') }}</text>
        </view>
        <view class="plate-input-body">
          <view class="plate-input-content" v-if="queryWay == 'room'">
            <input type="text" class="input-ui" v-model="roomNum" :placeholder="`${$t('请输入房号-RzA')}${$t('楼栋-tUB')}-${$t('单元-7xd')}-${$t('房屋-5Ci')} ${$t('如-i9f')}:1-1-1001`" />
          </view>
          ...
          <view class="flex justify-between margin-top msg-code" v-if="config.smsValidate == 'ON'">
            <input v-model="msgCode" :placeholder="$t('请输入短信验证码-YxH')" name="input" style="width: 70%" class="input-ui" />
            <button class="cu-btn bg-green shadow msg-code" :disabled="btnDisabled" @click="_sendSms()">{{ $t('验证码-dkC') }}</button>
          </view>
        </view>
      </view>
      ...
      <view class="cu-bar btn-group" style="margin-top: 30px">
        <button @click="_queryOwnerInfo" class="cu-btn bg-green shadow-blur round lg">{{ $t('立即查询-ygB') }}</button>
      </view>

      <view class="temp-remark">
        <view>{{ $t('支付说明-4qo') }}</view>
...
import { sendMsgCode, getPayFeeQrcode, getQrcodeOwner } from '@/api/fee/qrCodePayFee.js'
