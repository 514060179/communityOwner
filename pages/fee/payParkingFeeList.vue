      ...
      <view class="header_fixed" :style="{ top: headerTop }">
        <scroll-view v-if="parkingSpaces.length > 1" scroll-x class="bg-white nav" scroll-with-animation scroll-left="true">
          <view
            ...
            :class="item.psId == curParkingSpace.psId ? 'text-green cur' : ''"
            v-for="(item, index) in parkingSpaces"
            :key="index"
            @tap="switchParkingSpace(item)"
            :data-id="index"
          >
            {{ item.carNum }}({{ item.num }}{{ $t('车位-Imn') }})
          </view>
        </scroll-view>
      </view>
      <view v-if="parkingSpaces.length == 1" class="block__title">{{ parkingSpaces[0].carNum }}({{ parkingSpaces[0].num }}{{ $t('车位-Imn') }})</view>
      ...
      <view v-if="noData == false" class="margin-bottom-100">
        <view v-for="(item, index) in moreParkingSpaces" :key="index" class="bg-white margin-bottom margin-right-xs radius margin-left-xs padding-top padding-left padding-right">
          ...
          <view class="flex margin-top justify-between">
            <view class="text-gray">{{ $t('费用编码-5Cz') }}</view>
            <view class="text-gray">{{ item.feeId }}</view>
          </view>
          <view class="flex margin-top justify-between">
            <view class="text-gray">{{ $t('车牌号码-mCm') }}</view>
          ...
          <view class="flex margin-top-xs justify-between">
            <view class="text-gray">{{ $t('每月金额-7Ao') }}</view>
            <view class="text-gray">MOP {{ item.feePrice }}</view>
          </view>
          <view class="flex margin-top-xs justify-between">
            <view class="text-gray">{{ $t('到期时间-N71') }}</view>
          ...
          <view class="solid-top flex justify-end margin-top padding-top-sm padding-bottom-sm">
            ...
            <button class="cu-btn sm bg-green margin-left" @click="payFee(item)">{{ $t('缴费-OKv') }}</button>
          </view>
        </view>
      </view>
      <view v-else>
      ...
      context.request({
        ...
        success: function (res) {
          ...
          if (res.statusCode == 200) {
            //成功情况下跳转
            const _parkingSpaces = res.data.parkingSpaces
    ...
    _loadParkingSpaceFee: function () {
      ...
      context.request({
        ...
        success: function (res) {
          if (res.statusCode == 200) {
            //成功情况下跳转
            const _parkingSpaceFees = res.data.fees
            if (_parkingSpaceFees.length < 1) {
              _that.noData = true
            }
            _parkingSpaceFees.forEach(function (_fee) {
              const _tmpEndTime = _fee.endTime.replace(/\-/g, '/')
