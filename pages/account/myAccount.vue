  ...
  <view>
    <view class="tab-container bg-white">
      <view class="cu-list menu">
        <view class="cu-item arrow" v-for="(item, key) in accounts" :key="key" :data-item="item" @click="myAccountDetail(item)">
          <view class="content padding-tb-sm">
            <view> <text class="cuIcon-rechargefill text-green margin-right-xs"></text> {{ item.acctTypeName }} </view>
            <view class="text-gray text-sm"> <text class="cuIcon-right margin-right-xs"></text>{{ $t('余额-7gB') }}: {{ item.amount }} </view>
          </view>
          <view class="action"> </view>
        </view>
        <view class="cu-item arrow" @click="coupons(item)">
          <view class="content padding-tb-sm">
            <view> <text class="cuIcon-ticket text-green margin-right-xs"></text>{{ $t('我的优惠券-3eF') }} </view>
    ...
    <view class="cu-bar btn-group" style="margin-top: 30px">
      <button @click="_toPrestoreAccount" class="cu-btn bg-orange shadow-blur round lg">{{ $t('账户充值-ubZ') }}</button>
    </view>
  </view>
</template>
