<template>
  <view>
    <view class="block__title">{{ $t('账户明细-HMv') }}</view>
    <view v-if="accountDetails.length > 0">
      <view v-for="(item, index) in accountDetails" :key="index" class="bg-white margin-bottom margin-right-xs radius margin-left-xs padding">
        <view class="flex padding-bottom-xs solid-bottom justify-between">
          <view>{{ acctType == 2004 ? $t('积分-rik') : $t('金额-LrA') }}</view>
          <view class="text-gray">{{ item.amount }}{{ acctType == 2004 ? '分' : '元' }}</view>
        </view>
        <view class="flex margin-top justify-between">
          <view class="text-gray">{{ $t('交易编号-cle') }}</view>
          <view class="text-gray">{{ item.orderId }}</view>
        </view>
        <view class="flex margin-top-xs justify-between">
          <view class="text-gray">{{ $t('明细编号-WjJ') }}</view>
          <view class="text-gray">{{ item.detailId }}</view>
        </view>
        <view class="flex margin-top-xs justify-between">
          <view class="text-gray">{{ $t('账户名称-cWP') }}</view>
          <view class="text-gray">{{ item.acctName }}</view>
        </view>
        <view class="flex margin-top-xs justify-between">
          <view class="text-gray">{{ $t('明细类型-SVd') }}</view>
          <view class="text-gray">{{ item.detailType == '1001' ? $t('转入-p9W') : $t('转出-2wL') }}</view>
        </view>
        <view class="flex margin-top-xs justify-between">
          <view class="text-gray">{{ $t('交易时间-U66') }}</view>
          <view class="text-gray">{{ item.createTime }}</view>
        </view>
        <view class="flex margin-top-xs justify-between">
          <view class="text-gray">{{ $t('说明-xo8') }}</view>
