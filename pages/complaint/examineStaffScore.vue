<template>
  <view>
    <view class="flex justify-start ess-1">
      ...
      <view class="ess-1-s">
        <view class="">
          <text class="ess-1-s-a">{{ $t('姓名-rTx') }}:</text>
          <text class="ess-1-s-b">{{ staff.staffName }}</text>
        </view>
        <view class="margin-top-sm">
          <text class="ess-1-s-a">{{ $t('岗位-W4f') }}:</text>
          <text class="ess-1-s-b">{{ staff.post }}</text>
        </view>
        <view class="margin-top-sm">
          <text class="ess-1-s-a">{{ $t('工号-2or') }}:</text>
    ...
    <view class="bg-white ess-2">
      ...
      <view class="ess-2-btn">
        <button class="cu-btn round bg-primary" @click="_submitScore()">{{ $t('确定评分-uc6') }}</button>
      </view>
    </view>
    <view class="margin-top ess-3-introduction">{{ $t('简介-wqH') }}</view>
