  ...
  <view>
    <scroll-view scroll-x class="bg-white nav">
      <view class="flex text-center">
        <view class="cu-item flex-sub" :class="active == 0 ? 'text-green cur' : ''" @tap="tabSelect(0)"> {{ $t('正在处理-HJF') }} </view>
        <view class="cu-item flex-sub" :class="active == 1 ? 'text-green cur' : ''" @tap="tabSelect(1)"> {{ $t('处理完成-IIv') }} </view>
      </view>
    </scroll-view>
      ...
      <view v-if="noData == false">
        <view v-for="(item, index) in waitComplaint" :key="index" class="bg-white margin-top margin-right-xs radius margin-left-xs padding">
          ...
          <view class="flex margin-top justify-between">
            <view class="text-gray">{{ $t('投诉类型-gyd') }}</view>
            <view class="text-gray">{{ item.typeName }}</view>
          </view>
          <view class="flex margin-top-xs justify-between">
            <view class="text-gray">{{ $t('投诉人-iAs') }}</view>
            <view class="text-gray">{{ item.complaintName }}</view>
          </view>
          <view class="flex margin-top-xs justify-between">
            <view class="text-gray">{{ $t('所属房间-lQo') }}</view>
          ...
          <view class="solid-top flex justify-end margin-top padding-top-sm padding-bottom-sm">
            <button class="cu-btn sm line-gray" @click="_toComplaintDetail(item)">{{ $t('详情-6NZ') }}</button>
          </view>
        </view>
      </view>
      <view v-else>
    ...
    <view v-if="active == 1" class="ppfl_c">
      <view v-if="noData == false">
        <view v-for="(item, index) in complaintd" :key="index" class="bg-white margin-top margin-right-xs radius margin-left-xs padding">
          ...
          <view class="flex margin-top-xs justify-between">
            <view class="text-gray">{{ $t('投诉时间-k39') }}</view>
            <view class="text-gray">{{ item.createTime }}</view>
          </view>
          <view class="flex margin-top-xs justify-between">
            <view class="text-gray">{{ $t('投诉内容-O2S') }}</view>
            <view class="text-gray">{{ item.context }}</view>
          </view>
          <view class="solid-top flex justify-end margin-top padding-top-sm padding-bottom-sm">
            <button class="cu-btn sm bg-green" @click="_appraiseComplaint(item)">{{ $t('评价-ldp') }}</button>
            <button class="cu-btn sm line-gray margin-left" @click="_toComplaintDetail(item)">{{ $t('详情-6NZ') }}</button>
          </view>
        </view>
      </view>
      <view v-else>
...
import { getComplaints } from '@/api/community/complaintApi.js'
...
export default {
  ...
  methods: {
    ...
    _toComplaintDetail: function (_item) {
      context.navigateTo({
        url: '/pages/complaint/complaintDetail?complaintId=' + _item.complaintId + '&communityId=' + _item.communityId
      })
    },
    _appraiseComplaint: function (_item) {
      context.navigateTo({
        url: '/pages/complaint/appraiseComplaint?complaintId=' + _item.complaintId + '&communityId=' + _item.communityId
      })
    }
  }
}
