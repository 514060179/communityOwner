  ...
  <view>
    ...
    <view class="cu-bar search bg-white nav-list">
      <view class="search-form round" @tap="toSearch()">
        ...
        <input :adjust-position="false" type="text" :placeholder="$t('请输入商品名-mFH')" @tap="toSearch()" />
      </view>
      <view class="text-white" @tap="toSelectArea()"> {{ selectCommunityName }}<text class="cuIcon-refresharrow"></text> </view>
    </view>
    <!-- 店铺类型 -->
    <view class="">
      <scroll-view scroll-x class="nav nav-scroll padding-bottom-xs" scroll-with-animation>
        <view class="cu-item text-white" :class="item.shopTypeId == shopTypeId ? 'nav-cur' : ''" v-for="(item, index) in navList" :key="index" @tap="selectType(index)" :data-id="index">
          {{ item.typeName }}
        </view>
      </scroll-view>
    </view>
...
import { getMallIndexCategoryList, queryShopType } from '../../api/mall/mallApi.js'
import { getMallCommunityName } from '../../api/community/communityApi.js'
import { sliceArray } from '../../lib/proprietor/utils/ArrayUtil.js'
...
export default {
  ...
  methods: {
    selectType(index) {
      const shopTypeId = this.navList[index].shopTypeId
    ...
    _getCategoryList() {
      ...
      getMallIndexCategoryList(params).then(function (result) {
      ...
      queryShopType(params).then(function (result) {
        _that.navList = result.data
        _that.navList.unshift({
          shopTypeId: '0',
          typeName: this.$t('首页-6HF')
        })
      })
    },
