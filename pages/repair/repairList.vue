<template>
  <view class="user-container">
    <view v-for="(item, idx) in tableData" :key="idx" class="notice" :data-item="item" @tap="gotoDetail">
      ...
      <view class="main">
        <view>
          <view class="text">{{ $t('报修类型-qnU') }}: {{ item.repairTypeName }}</view>
          <view class="text">{{ $t('状-SV8') }}<text decode="true">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</text>{{ $t('态-Kqy') }}: {{ item.stateName }}</view>
          <view class="text">{{ $t('预约时间-FzQ') }}: {{ item.appointmentTime }}</view>
        </view>
        <view>
          <button class="button" size="mini" type="default" @tap.native.stop="gotoDetail" :data-item="item">{{ $t('查看-sbN') }}</button>
        </view>
      </view>
    </view>
  </view>
