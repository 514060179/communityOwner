      ...
      <view class="cu-item">
        ...
        <view class="content">
          <view class="text-grey">
            ...
            <view class="cu-tag round bg-orange sm">{{ $t('管理员-47S') }}</view>
          </view>
          <view class="text-gray text-sm">
            <view class="text-cut">
              <text class="margin-right-xs"></text>
              {{ startTime }}
              <text class="iconfont iconyuedu padding-left"></text> {{ readCount }} <text class="iconfont icongood-fill padding-left"></text> {{ likeCount }}
            </view>
          </view>
        </view>
      </view>
...
<script>
import { recoveryCommunityInfo } from '../../api/community/communityApi.js'
...
export default {
  ...
  onShow: function () {
    ...
    recoveryCommunityInfo(this).then(_communityInfo => {
  ...
  methods: {
    loadOwnerHeaderImg: function () {
      ...
      getCurOwner().then(_owner => {
        const _headerImg = url.getOwnerPhotoPath + '?objId=' + _owner.memberId + '&communityId=' + _owner.communityId + '&fileTypeCd=10000&time=' + new Date()
    ...
    loadActiviteFun: function () {
      ...
      // #endif
      loadActivites(_objData).then(_acts => {
        const _activites = _acts[0]
