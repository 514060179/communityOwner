  ...
  <view>
    ...
    <view class="VerticalBox">
      ...
      <scroll-view class="VerticalNav nav" scroll-y scroll-with-animation>
        <view class="cu-item" :class="item.catalogId == curCatalog.catalogId ? 'text-green cur' : ''" v-for="(item, index) in catalogs" :key="index" @tap="TabSelect(item)" :data-id="index">
          {{ item.name }}
        </view>
      </scroll-view>
          ...
          <view class="cu-list menu-avatar">
            <view class="cu-item vc-cu-item margin-bottom-xs" v-for="(item, index) in goods" :key="index" :id="'main-' + index">
              ...
              <view class="action">
                ...
                <view>
                  <button class="cu-btn round line-red sm" @click="_reserveDining(item)">{{ $t('预约-JJk') }}</button>
                </view>
              </view>
            </view>
          </view>
    ...
    <view class="bg-white border flex justify-between" style="position: fixed; width: 100%; bottom: 0">
      <view class="action text-red line-height margin-left" @click="_showSelectdGoods"> {{ $t('预约-JJk') }} {{ selectGoods.length }} {{ $t('件商品-roW') }} </view>
      <view class="flex justify-end">
        <view class="action text-orange margin-right line-height"> {{ $t('合计-87u') }}：MOP{{ receivableAmount }} </view>
        <view class="btn-group">
          <button class="cu-btn bg-red shadow-blur lgplus sharp" :disabled="receivableAmount == 0" @click="_toReserveOrder()">{{ $t('支付-lt5') }}</button>
        </view>
      </view>
    </view>
  ...
  methods: {
    ...
    computeGoodsPrice: function () {
      ...
      this.selectGoods.forEach(_goods => {
        const _quantity = _goods.hours.length * parseFloat(_goods.quantity)
