...
<template>
  <view class="">
    <view>
      <image :src="servInfo.imgUrl ? servInfo.imgUrl : noPic" />
    </view>

    <view class="introduce-section">
      <view class="price-box">
        <text class="price">MOP {{ servInfo.price }}</text>
        <text class="price-m">{{ $t('起-tNI') }}</text>
      </view>
    ...
    <view class="page-bottom">
      ...
      <view class="action-btn-group">
        <button type="primary" class="action-btn no-border add-cart-btn" @click="toOrder">{{ $t('预约下单-CP3') }}</button>
      </view>
    </view>

    <reserve-service-now ref="reserveServiceNowRef" :endDate="servInfo.endDate"></reserve-service-now>
  </view>
</template>
