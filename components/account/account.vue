    ...
    <view class="cu-list menu">
      <view class="cu-item arrow" @click="_toPrestoreAccount()">
        <view class="content padding-tb-sm">
          <view> <text class="cuIcon-rechargefill text-green margin-right-xs"></text> {{ account.acctTypeName }} </view>
          <view class="text-gray text-sm"> <text class="cuIcon-right margin-right-xs"></text>{{ $t('余额-7gB') }}: {{ account.amount }} </view>
        </view>
        <view class="action">
          {{ $t('去充值-aI4') }}
        </view>
