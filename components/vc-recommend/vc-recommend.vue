      ...
      <view class="text-left">
        <scroll-view scroll-x class="nav" scroll-with-animation>
          <view
            ...
            v-for="(item, index) in mainCatagorys"
            :key="index"
            @tap="_doSelect(item)"
            :data-id="index"
          >
        ...
        <view class="margin-top-xs margin-bottom-sm" v-for="(product, i) in products" :key="i">
          <view class="padding-sm bg-white goods" :class="i % 2 == 0 ? 'margin-right-xs' : 'margin-left-xs'" @tap="_toGoodsDetail(product)">
            ...
            <view class="flex justify-between margin-top-xs">
              <view>
                <text class="text-red">MOP{{ product.price }}</text>
              </view>
              <view>
                <text class="text-gray">{{ $t('销量-6wn') }}{{ product.sales }}{{ $t('件-dPx') }}</text>
              </view>
            </view>
          </view>
        </view>
...
<script>
import { queryMainCategory, queryPhoneMainCategoryProduct } from '../../api/goods/goodsApi.js'
