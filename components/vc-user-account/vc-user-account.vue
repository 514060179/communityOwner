<template>
  <view>
    <view v-if="accounts.length > 0">
      <view class="block__title">{{ $t('业主账户-Yf3') }}</view>
      <checkbox-group @change="checkboxChange">
        <view class="cu-list menu" v-for="(account, idx) in accounts" :key="idx" :data-item="account" @click="_viewAccountDetail(account)">
          <view class="cu-item">
            <view class="content padding-tb-sm">
              <view>
                <view class="text-cut" style="width: 220px"> <checkbox :value="account.acctId" :checked="account.checked" /> {{ account.acctTypeName }} </view>
              </view>
            </view>
            <view class="action">
              <text class="text-grey text-sm">MOP{{ account.amount }}</text>
            </view>
          </view>
        </view>
      </checkbox-group>
