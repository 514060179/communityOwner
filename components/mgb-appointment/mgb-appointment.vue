    ...
    <view>
      ...
      <form>
        <view class="cu-form-group">
          <view class="title">{{ $t('选择场馆-w2l') }}</view>
          <picker @change="PickerChange" :value="selectChangGuan" :range="picker">
            <view class="picker">
              {{ selectChangGuan > -1 ? picker[selectChangGuan] : '' }}
            </view>
          </picker>
        </view>
      </form>
      ...
      <view style="position: sticky; top: 0ch; z-index: 999">
        <scroll-view class="bg-white nav" scroll-with-animation scroll-x>
          <view class="scroll-view-item_H" @click="SelectHeader(item, i)" v-for="(item, i) in WeekList" :key="i" :class="selectHeader == i ? 'bg-cyan text-shadow' : 'bg-primary'">
        ...
        <view class="barConten">
          <view class="flex justify-between">
            <view class="radius text-df">{{ $t('选择预约时间段-DfE') }}</view>
            <view class="radius text-sm">{{ $t('共-ysv') }}{{ CDList.length }}{{ $t('场地-VP1') }}</view>
          </view>
        </view>
      </view>
      ...
      <scroll-view scroll-with-animation scroll-y>
        ...
        <view class="BiaoGe">
          ...
          <ul style="display: block; white-space: nowrap">
            <li v-if="CDList.length <= 0" style="display: inline-block; margin-left: 20px; margin-top: 20px; font-size: 14px">{{ $t('暂无订场信息-5Xl') }}</li>
            <li style="display: inline-block" v-for="(item, index) in CDList" :key="index">
              <view>
                ...
                <view style="background-color: #ffffff">
                  <view :class="'Fang ' + ChangDiStatus(item_x)" @click="SelectCd(item_x, index, open_x)" v-for="(item_x, open_x) in item.openTimes" :key="open_x">
        ...
        <view style="border-top: #dedede 1px solid; background-color: white">
          <view class="dashed-bottom p-3" style="padding-bottom: 5px; padding-left: 10px; padding-top: 5px">
            <view class="cu-capsule radius">
              ...
              <view class="cu-tag line-grey">{{ $t('不可约-HH1') }}</view>
            </view>
            <view class="cu-capsule radius">
              <view class="cu-tag bg-white" style="border: solid 1px #b8b8b8">-</view>
              <view class="cu-tag line-grey">{{ $t('可约-zy5') }}</view>
            </view>
            <view class="cu-capsule radius">
              <view class="cu-tag bg-primary" style="background-color: #8799a3">-</view>
              <view class="cu-tag line-grey">{{ $t('已有约-f1J') }}</view>
            </view>
            <view class="cu-capsule radius">
              <view class="cu-tag bg-cyan">-</view>
              <view class="cu-tag line-grey">{{ $t('已选择-FFV') }}</view>
            </view>
          </view>
            ...
            <ul class="flex-imd">
              <li v-for="(item, index) in CurList" :key="index" class="navbar-item">
                <view style="line-height: 15px">
                  <view style="width: 84px; text-align: center; padding-top: 3px; background-color: #1cbbb4; height: 23px; color: #ffffff; font-size: 10px">
                    {{ HoursStr(item.hours) }}
                  </view>
                  <view style="padding-top: 3px; text-align: center; background-color: white; height: 23px; font-size: 14px">
                    {{ item.changdiName }}
                  </view>
                </view>
              </li>
          ...
          <view class="cu-form-group">
            ...
            <button @click="confCd()" class="cu-btn round bg-primary" style="background-color: #1cbbb4">{{ $t('提交订单-sDX') }}</button>
          </view>
        </view>
      </view>
    </view>
...
import { getChangdi, getChangGuan, getBetweenTime } from '@/api/appointment/appointmentApi.js'
import { getBooks } from '../../api/booking/bookingApi.js'
...
export default {
  ...
  methods: {
    ...
    initTimeLine() {
      this.TimeList = []
      const startTime = this.CGList[this.selectChangGuan]['startTime']
      const endTime = this.CGList[this.selectChangGuan]['endTime']
    ...
    _loadChangdiList() {
      ...
      getChangdi(params).then(function (result) {
        // debugger
        wx.hideLoading()
        if (result.data.length > 0) {
          const Data = that._filtersChangdiTime(result.data)
          that.CDList = Data
    ...
    _loadIsBookList() {
      ...
      getBooks(1).then(function (data) {
        ...
        // debugger
        const _bookings = data.data
        const _bookingList = []

        for (var col = 0; col < _bookings.length; col++) {
          const _times = _bookings[col].times
        ...
        for (var clindex = 0; clindex < that.CDList.length; clindex++) {
          const _item = that.CDList[clindex]
          _item.openTimes.forEach(function (eItem) {
            if (eItem.isOpen == 'Y') {
              ...
              const da = that.selectDate + ' ' + h + ':00:00'
              const nowD = new Date()
              const itemD = new Date(da)
              ...
              if (nowD.getTime() > itemD.getTime()) {
                ...
                _bookingList.forEach(function (bkItem) {
                  if (bkItem.spaceId == eItem.spaceId && bkItem.hours == eItem.hours && bkItem.appointmentTime == that.selectDate && bkItem.state != 'Y') {
                    // debugger
                    eItem.isOpen = 'S'
                    // debugger;
                    return
    ...
    _filtersChangdiTime(d) {
      for (var col = 0; col < d.length; col++) {
        const f = d[col].openTimes
        const g = []
        // 循环 openTimes 判断 每个场地的时间
        for (var cola = 0; cola < f.length; cola++) {
          //为保持一致判断
          const hoursDouble = f[cola].hours < 10 ? '0' + f[cola].hours : f[cola].hours
          const h = hoursDouble + ':00'
          let k = 'false'
    ...
    _loadChangGuan() {
      ...
      getChangGuan(params).then(function (result) {
    ...
    SelectCd(obj, changdi, cdIndex) {
      const cdMoney = Number(this.CDList[changdi].feeMoney) // 场地的费用
      ...
      if (obj.isOpen === 'C') {
        ...
        const index_1 = this.CurList.findIndex(item_1 => {
          if (item_1 == obj) {
            return true
          }
        })
  ...
  mounted() {
    ...
    const _that = this
    this.vc.getCurCommunity().then(function (_communityInfo) {
