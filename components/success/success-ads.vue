<template>
  <view>
    <view class="home_swiper" v-if="ad && ad.length > 0">
      <text class="text-gray padding-xs">{{ $t('更多精彩-EQN') }}</text>
      <swiper
        class="ad-swiper margin-top-xs bg-white"
        indicator-dots="true"
...
<script>
import { listMarketPicByCommunity } from '../../api/index/indexApi.js'
...
export default {
  ...
  methods: {
    _loadAdvertPhoto: function () {
      ...
      //查询 广告
      listMarketPicByCommunity(_objData).then(function (_aPhotos) {
        _that.ad = _aPhotos.data
      })
    },
    _advertJump: function (ad) {
      // 站外
      const url = encodeURIComponent(ad.picLink)
      uni.navigateTo({
