<template>
  <view class="vc-shop margin-top" v-if="shops && shops.length > 0">
    <view class="text-left margin-left-xs margin-bottom-sm">
      <text class="recommend-title">{{ $t('附近商家-eYx') }}</text>
    </view>
      ...
      <view class="cu-item cur" @click="_toShopDetail(item)">
        ...
        <view class="content">
          ...
          <view class="text-gray text-sm flex">
            <view class="text-cut"> <text class="cuIcon-locationfill text-orange margin-right-xs"></text> {{ item.sendAddress }} </view>
          </view>
        </view>
...
<script>
import { loadCommunityShops } from '../../api/mall/mallApi.js'
...
export default {
  ...
  methods: {
    _loadCommunityShops: function () {
      ...
      loadCommunityShops(_data).then(_shops => {
        _that.shops = _shops
      })
    },
    /**
    ...
    _toShopDetail: function (_shop) {
      const url = this.openType == '1,3' ? '/pages/cate/cate?shopId=' : '/pages/homemaking/shop?shopId='
