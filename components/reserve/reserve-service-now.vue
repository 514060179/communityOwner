<template>
  <view class="">
    <!-- 规格 -->
    <view class="cu-modal sku-modal bottom-modal" style="z-index: 999" :class="{ show: isShow }" @tap="_cancle">
      <view class="cu-dialog" @tap.stop style="background: none">
        <view class="shop-modal page_box">
          ...
          <view class="flex justify-between margin-top">
            <view>{{ $t('预约时间-FzQ') }}</view>
            <view style="width: 60%">
              <checkbox-group class="text-right" @change="_changeReserveTime">
                <view class="margin-right-sm" style="display: inline-block" v-for="(item, index) in openTimes" :key="index">
                  <block v-if="item.isOpen == 'Y'">
                    <checkbox class=" " :value="item.hours"></checkbox>
                    <view>{{ item.hours }}{{ $t('时-ExD') }}</view>
                  </block>
                </view>
              </checkbox-group>
              <!--  -->
          ...
          <view class="btn-box foot_box margin-top-lg">
            <button class="cu-btn seckill-btn" @tap="confirm">{{ $t('确认-Hp3') }}</button>
          </view>
        </view>
      </view>
    </view>
...
<script>
  import { getCatalogGoodss, getReserveParams } from '@/api/community/reserveApi.js'
    ...
    methods: {
      ...
      _loadGoods: function(_goodsId) {
        ...
        getCatalogGoodss({
          ...
          _data[0].hoursMaxQuantity = parseInt(Number(_data[0].hoursMaxQuantity))
          _that.goods = _data[0]
          _that.goods.hours = []
      ...
      _loadGoodsParams: function() {
        ...
        getReserveParams({
          ...
          const hasOpen = _data[0].openTimes.some(v => v.isOpen == 'Y')
          if (!hasOpen) {
            uni.showToast({
              icon: 'none',
              title: _that.$t('当前没有开放的预约时间段-MCo')
            })
          }
      ...
      _computeMoney: function() {
        const _quantity = this.goods.hours.length * parseFloat(this.goods.quantity)
      ...
      confirm: function() {
        const that = this
        // 没有选择预约时间
        if (!this.goods.hours.length) {
          uni.showToast({
            icon: 'none',
            title: that.$t('请选择预约时间-DvZ')
          })
          return 
        }
        const _selectGoods = []
        ...
        uni.navigateTo({
          url: `/pages/reserve/reserveOrder?from=service&endDate=${this.endDate}`
        })
      }
    }
  }
