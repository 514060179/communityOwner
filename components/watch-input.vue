<template>
  <view class="main-list oBorder">
    ...
    <!-- 是否可见密码 -->
    <image v-if="_isShowPass && type === 'password' && !_isShowCode" class="img cuIcon" :class="showPassword ? 'cuIcon-attention' : 'cuIcon-attentionforbid'" @tap="showPass" />
    <!-- 倒计时 -->
    <view class="vercode" v-if="_isShowCode && !_isShowPass" :class="{ 'vercode-run': second > 0 }" @click="setCode">{{ getVerCodeSecond }}</view>
  </view>
</template>
...
export default {
  ...
  computed: {
    ...
    getVerCodeSecond() {
      //验证码倒计时计算
      if (this.second <= 0) {
        return this.codeText || this.$t('获取验证码')
      } else {
        if (this.second < 10) {
