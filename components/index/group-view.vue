<template>
  <view class="bg-white group-view margin-top">
    <view class="flex justify-between group-view-top">
      <view>
        <text class="view-title">{{ $t('超值拼团-SIN') }}</text>
        <text class="view-sub-title">{{ $t('拼着买更便宜-iVF') }}</text>
      </view>
      <view>
        <text @tap="_toGroupGoodsList" class="view-title-more">{{ $t('更多-Miu') }} ></text>
      </view>
    </view>
    <view class="bg-white flex justify-between group-goods">
      <view v-for="(item, index) in goods" :key="index" class="group-item" @click="_toGroupGoods(item)">
        ...
        <view>
          <text class="group-price">MOP {{ item.groupPrice }}</text>
        </view>
        <view>
