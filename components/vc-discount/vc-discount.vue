<template>
  <view>
    <view v-if="discounts && discounts.length > 0">
      <view class="block__title">{{ $t('折扣信息-dDk') }}</view>

      <view class="cu-list menu" v-for="(discount, idx) in discounts" :key="idx" :data-item="discount" @click="_viewDiscountDetail(discount)">
...
import { getFeeDiscounts } from '../../api/fee/feeApi.js'
export default {
  components: {
    blockExplain
  },
