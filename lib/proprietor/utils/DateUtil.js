 ...
 * @param {Date对象} date 
 */
export function formatDate(date, joinStr) {
	const year = date.getFullYear();
	const month = date.getMonth() + 1;
	const day = date.getDate();
...
/**
 * @description 秒 转天-时-分-秒
 * @param {Object} _second 指定秒
 * @return {DD-HH-mm-ss} 返回
 */

export function secondToDateString(_second) {
  let second =  _second % 60 ;  // 秒
  let minute = Math.floor( _second / 60 % 60 );  // 分
  let hours = Math.floor( _second / 60 / 60  ) % 24;  // 小时
  let day = Math.floor( _second / 60 / 60 / 24 );  // 天
...
/**
 * @description 秒 转时-分-秒
 * @param {Object} _second 指定秒
 * @return {HH-mm-ss} 返回
 */

export function secondToHoursString(_second) {
  let second =  _second % 60 ;  // 秒
  let minute = Math.floor( _second / 60 % 60 );  // 分
  let hours = Math.floor( _second / 60 / 60  );  // 小时
...
export function secondToMinuteString(_second) {
  let second =  _second % 60 ;  // 秒
  let minute = Math.floor( _second / 60 );  // 分
